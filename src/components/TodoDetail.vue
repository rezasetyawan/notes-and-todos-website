        ...
        if (updatedTodoItems.length) {
          updatedTodoItems.forEach(async (todoItem) => {
            await updateTodoItemById(todoItem.id, {
              ...
              alert("success update todo item");
            });
          });
        }
  ...
  const deleteTodoItem = async (todoItemId: string) => {
    try {
      ...
      await deleteTodoItemById(todoItemId).then(() => {
        console.log(todoItemId + " deleted");
      });
    } catch (error) {
      console.error(error);
    }
  ...
  <template>
    ...
    >
        ...
        >
          <div
            class="p-3 pr-4 min-w-[500px] max-h-[500px] overflow-scroll relative bg-white rounded-md"
          >
